  ...
  $.fn.validEmail = (options) ->
    check = ($input) ->
      if $input.is("input,textarea")
        ...
        emailRegExp.test $input.val()
      else
        false
    applyCode = ($input) ->
      (if check($input) then success.call($input.get(0)) else failure.call($input.get(0)))
    options = options or {}
    on_ = options.on
    ...
    testInitially = options.testInitially or false
    $input = $(this)
    if typeof on_ is "string"
      $input.bind on_, ->
        applyCode $(this)
